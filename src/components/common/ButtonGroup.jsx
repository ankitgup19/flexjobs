...
const ButtonGroup = (props) => {
    ...
    const changeHandler = () => {
        setCount([]);
        document.querySelectorAll(".btn-group.active input[type=checkbox]:checked").forEach((e)=>{
            count.push(e.value);
        });
    ...
    return (
        <div className = {`btn-group ${props.active ? "active" : ""}`}>
            {props.name && <Button onClick={props.onToggle} className={activeCount > 0 && "active"}>{props.name}{activeCount > 1 && <span className="count">({activeCount})</span>}</Button>}
            {
